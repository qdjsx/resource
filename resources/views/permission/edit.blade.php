                ...
                <div class="layui-input-inline">
                    <select name="menu_id" lay-verify="required" lay-search="" lay-filter="template">
                        ...
                        @foreach ($parents as  $v)
                            <option value="{{$v->id}}"  @if($item->menu_id == $v->id) selected @endif>{{$v->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div class="layui-input-inline">
                    <select name="function" lay-verify="required" lay-search="" lay-filter="template">
                        ...
                        @foreach ($function as  $k => $v)
                            <option value="{{$k}}" @if($item->function == $k) selected @endif>{{$v}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block" style="margin-left: 180px;">
                <input name="controller" id="title" lay-verify="required" placeholder="请输入控制器" autocomplete="off"
                       class="layui-input" type="text" value="{{$item->controller}}">
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block" style="margin-left: 180px;">
                <input name="action" id="title" lay-verify="required" placeholder="请输入action" autocomplete="off"
                       class="layui-input" type="text" value="{{$item->action}}">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label" style="width: 150px;">是否匹配参数:</label>
            <div class="layui-input-block" style="margin-left: 180px;">
                <input type="radio" name="is_check" value="1" title="是" @if($item->is_check == 1) checked="" @endif>
                <input type="radio" name="is_check" value="0" title="否" @if($item->is_check == 0) checked="" @endif>
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block" style="margin-left: 180px;">
                <input name="params" id="title" lay-verify="" placeholder="" autocomplete="off"
                       class="layui-input" type="text" value="{{$item->params}}">
            </div>
        </div>
    ...
    <script>
        layui.use(['form'], function(){
            ...
            form.on('submit(submit)', function(data){
                $.post("{{url('permission/update')}}/{{$item->id}}",$("#form_edit").serialize(),function(data) {
                    layer.load();
                    if(data.code == 200) {
