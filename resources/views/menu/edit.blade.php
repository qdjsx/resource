        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block" style="margin-left: 180px;">
                <input name="url" id="title" lay-verify="required" placeholder="菜单地址" autocomplete="off"
                       class="layui-input" type="text" value="{{$item->url}}">
            </div>
        </div>
        <div class="layui-form-item">
                ...
                <div class="layui-input-inline">
                    <select name="parent_id" lay-verify="required" lay-search="" lay-filter="template">
                        ...
                        @foreach ($parents as  $v)
                            <option value="{{$v->id}}" @if ($item->parent_id == $v->id) selected @endif>{{$v->name}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div class="layui-input-inline">
                    <select name="level" lay-verify="required" lay-search="" lay-filter="template">
                        ...
                        @foreach ($levelArr as  $k =>$v)
                            <option value="{{$k}}" @if ($item->level == $k) selected @endif>{{$v}}</option>
                        @endforeach
                    </select>
                </div>
            </div>
    ...
    <script>
        layui.use(['form'], function(){
            ...
            form.on('submit(submit)', function(data){
                $.post("{{url('menu/update')}}/{{$item->id}}",$("#form_edit").serialize(),function(data) {
                    layer.load();
                    if(data.code == 200) {
