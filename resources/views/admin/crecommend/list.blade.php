...
@section('body_content')
    ...
    <script>
        ...
        layui.use(['table', 'laydate'], function () {
            ...
            table.render({
                elem: '#demo',
                url: '{{url("couponR/ajaxList")}}?_token={{csrf_token()}}',
                method: 'post',
                loading: true,
                skin: 'line',
                page: true,
                ...
                cols: [[ //标题栏
                    ...
                    , {field: 'is_hot_sell', title: '是否热销'}
                    , {field: 'valid_date', title: '截止时间'}
            ...
            table.on('tool(demoTable)', function (obj) {
               ...
               if (obj.event === 'edit') {
                    layer.open({
                        title: data.coupon_name + '-编辑',
                        type: 2,
                        skin: 'layui-layer-molv',
                        area: ['60%', '80%'],
                        maxmin: true,
                        content: '{{url("crecommend/edit")}}/' + data.id + '/' + $.md5("{{(env('APP_KEY'))}}" + data.id)
                    });
                }else if (obj.event === 'del') {
                   layer.confirm('真的删除行么', function (index) {
                       $.post("{{url('crecommend/del')}}/" + data.id + '/' + $.md5("{{(env('APP_KEY'))}}" + data.id), '_token={{csrf_token()}}', function (data) {
                           if (data.code == 200) {
            ...
            $('#add_button').click(function(){
                layer.open({
                    ...
                    area: ['60%', '80%'],
                    maxmin: true,
                    content: '{{url('crecommend/create')}}?_token={{csrf_token()}}'
                });
            });
