...
@section('body_content')
    ...
    <form class="layui-form"  id="form_add">
            ...
            <div class="layui-input-block">
                <select name="channel_id" id="channel_id" lay-verify="required" lay-search="">
                    ...
                    @foreach($channels as  $channel)
                        <option value="{{$channel->id}}" @if($item->channel_id == $channel->id) selected @endif>{{$channel->title}}</option>
                    @endforeach
                </select>
            </div>
        </div>
            ...
            <div class="layui-input-block">
                <select name="coupon_id" id="coupon_id" lay-verify="required" lay-search="">
                    ...
                    @foreach($coupons as  $coupon)
                        <option value="{{$coupon->id}}" @if($item->coupon_id == $coupon->id) selected @endif>{{$coupon->title}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="weight" lay-verify="number" value="{{$item->weight}}" placeholder="请输入" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">状态-默认开启</label>
            <div class="layui-input-block">
                @if ($item->status == 1)
        ...
        <div class="layui-form-item layui-form-text">
            ...
            <div class="layui-input-block">
                <textarea placeholder="请输入内容" class="layui-textarea" name="remark" id="remark">{{$item->remark}}</textarea>
            </div>
        </div>
        <div class="layui-form-item">
            <div class="layui-input-block">
    ...
    <script>
        layui.use(['form'], function(){
            ...
            form.on('submit(submit)', function(data){
                $.post("{{url('crecommend/update')}}/{{$item->id}}/{{md5(env('APP_KEY').$item->id)}}",$("#form_add").serialize(),function(data) {
                    layer.load();
                    if(data.code == 200) {
