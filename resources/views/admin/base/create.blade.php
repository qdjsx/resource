    ...
    <form class="layui-form"  id="form_add">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                @if($column['type'] == 'text')
                <input type="text"  name="{{$column['field']}}"  lay-verify="required"  class="layui-input">
                    @elseif($column['type'] == 'select')
                    <select name="{{$column['field']}}" id="{{$column['field']}}" lay-verify="required" lay-search="">
                        <option value="">全部</option>
                        @foreach ($column['values'] as $k =>$item)
                    ...
                    <input type="radio" name="add_params" value="1" title="是">
                    <input type="radio" name="add_params" value="0" title="否" checked="checked">
                    @endif

            </div>
        </div>
        @endforeach
        <input type="hidden" name="image_path" id="image_path" lay-verify="image_path">
    ...
    <script>
        layui.use(['form','upload'], function(){
            ...
            form.on('submit(submit)', function(data){
                $.post("{{url($url)}}",$("#form_add").serialize(),function(data) {
                    layer.load();
                    if(data.code == 200) {
                        layer.closeAll('loading');
                        layer.msg(data.msg, {
